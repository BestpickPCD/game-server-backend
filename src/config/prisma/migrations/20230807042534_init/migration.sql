...
CREATE TABLE `Balances` (
    `id` INTEGER NOT NULL AUTO_INCREMENT,
    `userId` INTEGER NOT NULL,
    `balances` DECIMAL(10, 2) NOT NULL DEFAULT 0,
    `currencyId` INTEGER NOT NULL,
    `updatedBy` INTEGER NULL,
    `createdAt` DATETIME(3) NOT NULL DEFAULT CURRENT_TIMESTAMP(3),
    `updatedAt` DATETIME(3) NULL,
...
-- CreateTable
CREATE TABLE `AgentVendorTokens` (
    `id` INTEGER NOT NULL AUTO_INCREMENT,
    `agentId` INTEGER NOT NULL,
...
CREATE TABLE `Vendors` (
    ...
    UNIQUE INDEX `Vendors_url_key`(`url`),
    PRIMARY KEY (`id`)
) DEFAULT CHARACTER SET utf8mb4 COLLATE utf8mb4_unicode_ci;
...
-- AddForeignKey
ALTER TABLE `Players` ADD CONSTRAINT `Players_agentId_fkey` FOREIGN KEY (`agentId`) REFERENCES `Agents`(`id`) ON DELETE RESTRICT ON UPDATE CASCADE;
...
-- AddForeignKey
ALTER TABLE `AgentVendorTokens` ADD CONSTRAINT `AgentVendorTokens_agentId_fkey` FOREIGN KEY (`agentId`) REFERENCES `Agents`(`id`) ON DELETE RESTRICT ON UPDATE CASCADE;

-- AddForeignKey
ALTER TABLE `AgentVendorTokens` ADD CONSTRAINT `AgentVendorTokens_vendorId_fkey` FOREIGN KEY (`vendorId`) REFERENCES `Vendors`(`id`) ON DELETE RESTRICT ON UPDATE CASCADE;
